...
public class Knapsack {
    ...
    // Get value for ith item given capacity c
    ...
    public int compute() {
        for (int i = 1; i < n; i++) {
            for (int c = 1; c <= W; c++) {
                ...
                int wi = w[i];
                if (wi <= c)
                    vi = max(v[i] + get(i - 1, c - wi), get(i - 1, c));
                else
                    vi = get(i - 1, c);
                put(i , c, vi);
            }
    ...
    public static void main(String[] args) {
        ...
        Y = new int[noOfItems + 1];
        X = new int[noOfItems + 1];
        ...
        Z = scanner.nextInt();                        // 10
        
        Knapsack knapsack = new Knapsack(Y, X, Z);
